...
// Database kodepromo
const promoCodes = {
    'DIMANA123': {discount:11, type:'percentage', description: 'Diskon 11%'},
    'APAANTUH': {discount:20, type:'percentage', description: 'Diskon 20%'},
    'WOWMANTAP': {discount:50000, type:'fixed', description: 'Diskon Rp.50,000'},
};
...
const themeToggleBtn = document.getElementById("theme-toggle"); // Mengubah dari darkModeToggle ke theme-toggle
...
function showPaymentModal(transaction) {
    // Menyesuaikan warna teks modal untuk dark mode
    const customerNameTextClass = htmlElement.classList.contains('dark') ? 'text-gray-300' : 'text-gray-600';
    const totalTextClass = htmlElement.classList.contains('dark') ? 'text-green-400' : 'text-green-600'; // Menyesuaikan warna total untuk dark mode

    paymentDetails.innerHTML = `
        <div class="space-y-2">
            <div class="flex justify-between">
                <span class="${customerNameTextClass}">ID Transaksi:</span>
                <span class="font-medium text-gray-800 dark:text-gray-100">${transaction.id}</span>
            </div>
            <div class="flex justify-between">
                <span class="${customerNameTextClass}">Nama:</span>
                <span class="font-medium text-gray-800 dark:text-gray-100">${transaction.customerName}</span>
            </div>
            <div class="flex justify-between">
                <span class="${customerNameTextClass}">Produk:</span>
                <span class="font-medium text-gray-800 dark:text-gray-100">${transaction.product}</span>
            </div>
            <div class="flex justify-between">
                <span class="${customerNameTextClass}">Jumlah:</span>
                <span class="font-medium text-gray-800 dark:text-gray-100">${transaction.quantity}</span>
            </div>
            <div class="flex justify-between">
                <span class="${customerNameTextClass}">Metode:</span>
                <span class="font-medium text-gray-800 dark:text-gray-100">${paymentMethodNames[transaction.paymentMethod]}</span>
            </div>
            ${transaction.discount > 0 ? `
            <div class="flex justify-between text-green-600 dark:text-green-400"> <span>Diskon:</span>
                <span class="font-medium">-${formatCurrency(transaction.discount)}</span>
            </div>
            ` : ''}
            <hr class="my-2 border-gray-200 dark:border-gray-600"> <div class="flex justify-between text-lg font-semibold">
                <span>Total:</span>
                <span class="${totalTextClass}">${formatCurrency(transaction.total)}</span>
            </div>
        </div>
    `;
...
// ===== FUNGSI DARK MODE =====
// Cek preferensi pengguna dari local storage atau sistem saat pertama kali memuat
